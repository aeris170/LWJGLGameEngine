...
public class ShadowMapMasterRenderer {
	 ...
	 */
	public ShadowMapMasterRenderer(final Camera camera) {
	...
	private void updateLightViewMatrix(final Vector3f direction, final Vector3f center) {
		...
		final float pitch = (float) Math.acos(new Vector2f(direction.x, direction.z).length());
	 ...
	 */
	private void updateOrthoProjectionMatrix(final float width, final float height, final float length) {
	...
	private static Matrix4f createOffset() {
		final Matrix4f offset = new Matrix4f();
